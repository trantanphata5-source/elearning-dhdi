...
class ApiService {
  ...
  async uploadPhoto(masv, photoBase64OrUrl) {
    ...
    let compressedPhoto = photoBase64OrUrl;
    if (photoBase64OrUrl && photoBase64OrUrl.startsWith('data:image')) {
      try {
        compressedPhoto = await this._compressImage(photoBase64OrUrl, 800, 0.7);
      } catch (compErr) {
    ...
    list[idx].anh3x4 = compressedPhoto;
    ...
    const currentUser = this.getCurrentUser();
    if (currentUser && currentUser.masv === masv) {
      currentUser.anh3x4 = compressedPhoto;
      currentUser.photoApproved = false;
      currentUser.photoStatus = 'CHỜ DUYỆT';
      localStorage.setItem(STORAGE_KEY_CURRENT_USER, JSON.stringify(currentUser));
    }

    // Sync to Google Apps Script (Google Drive + Google Sheet)
    ...
    if (this.apiUrl) {
      try {
        ...
        // GAS may redirect (302). Handle both ok and redirected responses.
        ...
        if (res.success && res.photoUrl) {
          // Replace local base64 with Google Drive URL from server
          list[idx].anh3x4 = res.photoUrl;
          list[idx].photoStatus = res.photoStatus || 'CHỜ DUYỆT';
          this.saveLocalStudents(list);
          if (currentUser && currentUser.masv === masv) {
            currentUser.anh3x4 = res.photoUrl;
            currentUser.photoStatus = res.photoStatus || 'CHỜ DUYỆT';
            localStorage.setItem(STORAGE_KEY_CURRENT_USER, JSON.stringify(currentUser));
          }
          syncSuccess = true;
        } else if (res.success) {
    ...
    return {
      success: true,
      message: syncSuccess 
        ? 'Đã tải lên ảnh 3x4 thành công và đồng bộ lên hệ thống! Ảnh đang trong trạng thái CHỜ DUYỆT bởi Ban Quản trị.'
        : 'Đã lưu ảnh 3x4 tạm thời. ' + (syncError ? '⚠️ Chưa đồng bộ được lên máy chủ (' + syncError + '). Vui lòng thử lại sau!' : 'Đang chờ đồng bộ...'),
