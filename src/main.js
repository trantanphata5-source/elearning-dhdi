...
function renderAdminView() {
  ...
  const currentApiUrl = api.getApiUrl();
    ...
    <div class="admin-container">
      <div class="admin-header">
        <div>
          ...
          <p style="color: var(--text-secondary); margin-top: 4px;">Duyệt ảnh thẻ 3x4 sinh viên, quản lý cấu hình kết nối máy chủ API và thông tin 5 lớp.</p>
        </div>
      ...
      <div class="admin-api-panel">
        <h3 style="font-size: 1.2rem; color: var(--text-title); margin-bottom: 8px;">
          <i class="fa-solid fa-link" style="color: #2563eb;"></i> Cấu Hình Kết Nối Máy Chủ API (Google Apps Script)
        </h3>
        <p style="color: var(--text-secondary); font-size: 0.9rem;">
          URL Web App kết nối cơ sở dữ liệu và tự động gửi Email tài khoản:
        </p>
        <div class="api-url-input-group">
          <input type="url" id="adminApiUrlInput" class="form-control" placeholder="https://script.google.com/macros/s/.../exec" value="${currentApiUrl}">
          <button class="btn btn-primary" onclick="saveAdminApiUrl()">
            <i class="fa-solid fa-floppy-disk"></i> Lưu Cấu Hình API
          </button>
        </div>
      </div>
...
window.openStudentDetailModal = function(masv) {
  const student = state.students.find(s => String(s.masv).trim() === String(masv).trim());
  if (!student) return;

  const title = document.getElementById('studentDetailTitle');
  const body = document.getElementById('studentDetailBody');
  const fullname = student.fullname || `${student.hodem || ''} ${student.ten || ''}`.trim();
